...
const app=express(); const PORT=10000;
...
const __filename="/tmp/sheliva-server.js"; const __dirname="/tmp";
const DATA="/tmp/sheliva-data", UP="/tmp/sheliva-uploads";
...
async function sendMail({to,subject,title,body,code=""}){
  ...
  const from=String(env.RESEND_FROM||"SHELİVA <onboarding@resend.dev>").trim();
...
app.post("/api/admin/login",(q,r)=>{
  const configured=String(env.ADMIN_SECRET||"");
